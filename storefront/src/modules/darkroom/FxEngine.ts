// src/modules/darkroom/fx/FxEngine.ts
...
function monoHalftone(out:CanvasRenderingContext2D, src:ImageData, p:Required<Pick<FxParams,"cell"|"gamma"|"minDot"|"maxDot"|"angle"|"shape">>) {
...
export async function renderFX(src: HTMLImageElement | HTMLCanvasElement, params: FxParams): Promise<HTMLCanvasElement> {
  ...
  const { canvas, ctx, imageData } = getImageDataFrom(src, p.previewScale)
  ...
  switch (p.method) {
    ...
    case "halftone-cmyk": {
      ...
      for (let i=0,j=0;i<D.length;i+=4,j++) L[j]=lum(D[i],D[i+1],D[i+2])
      ...
      octx.clearRect(0,0,out.width,out.height)
      // “цветной превью” — просто суммируем пластины с оттенками
