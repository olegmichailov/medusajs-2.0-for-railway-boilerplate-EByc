...
import {
  ...
  PanelRightOpen, PanelRightClose, Circle, Square, Triangle, Plus, Slash,
  Eye, EyeOff, Lock, Unlock, Copy, Trash2, ArrowUp, ArrowDown, Download
} from "lucide-react"
import type { ShapeKind, Side, Tool } from "./store"
import { isMobile } from "react-device-detect"

const SYS = "system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif"

// базовые классы
const wrap = "backdrop-blur bg-white/90 border border-black/10 shadow-xl rounded-none"
const ico  = "w-5 h-5"

// квадратные иконки прижаты друг к другу (gap-0)
const iconBtn =
  "h-10 grid place-items-center border border-black/80 rounded-none hover:bg-black hover:text-white transition " +
  "text-[11px] font-bold" // system-ui подтянется с родителя
...
type ToolbarProps = {
  ...
  setSelectedFontFamily: (f: string) => void // не используем, но оставляем для совместимости
...
export default function Toolbar(props: ToolbarProps) {
  ...
  if (!isMobile) {
    ...
    return (
      <div
        className={wrap + " fixed z-40 w-[380px] p-3"}
        style={{ left: pos.x, top: pos.y, fontFamily: SYS }}
      >
        <div className="flex items-center justify-between mb-3 cursor-move" onMouseDown={onDragStart}>
          <div className="text-[11px] uppercase tracking-wide font-bold">TOOLS</div>
          <div className="flex items-center gap-2">
            <button className={iconBtn + " w-10"} onClick={toggleLayers} title="Layers">
              {layersOpen ? <PanelRightClose className={ico}/> : <PanelRightOpen className={ico}/>}
            </button>
            <button className={iconBtn + " w-10"} onClick={() => setOpen(!open)} title="Collapse">{open ? "×" : "≡"}</button>
          </div>
        </div>

        {open && (
          <div className="space-y-3">
            {/* иконки-инструменты: прижаты (gap-0) */}
            <div className="grid grid-cols-6 gap-0 rounded-none overflow-hidden">
              <button className={clx(iconBtn, tool==="move"  && "bg-black text-white")}  onClick={()=>setTool("move")}  title="Move"><Move className={ico}/></button>
              <button className={clx(iconBtn, tool==="brush" && "bg-black text-white")} onClick={()=>setTool("brush")} title="Brush"><Brush className={ico}/></button>
              <button className={clx(iconBtn, tool==="erase" && "bg-black text-white")} onClick={()=>setTool("erase")} title="Eraser"><Eraser className={ico}/></button>
              <button className={iconBtn} onClick={onAddText} title="Text"><TypeIcon className={ico}/></button>
              <button className={clx(iconBtn, tool==="shape" && "bg-black text-white")} onClick={()=>setTool("shape")} title="Shapes"><Shapes className={ico}/></button>
              <button className={iconBtn} onClick={()=>fileRef.current?.click()} title="Image"><ImageIcon className={ico}/></button>
            </div>
            ...
            {tool==="shape" && (
              <>
                <div className="grid grid-cols-5 gap-0 rounded-none overflow-hidden">
                  <button className={iconBtn} onClick={()=>onAddShape("circle")}   title="Circle"><Circle className={ico}/></button>
                  <button className={iconBtn} onClick={()=>onAddShape("square")}   title="Square"><Square className={ico}/></button>
                  <button className={iconBtn} onClick={()=>onAddShape("triangle")} title="Triangle"><Triangle className={ico}/></button>
                  <button className={iconBtn} onClick={()=>onAddShape("cross")}    title="Cross"><Plus className={ico}/></button>
                  <button className={iconBtn} onClick={()=>onAddShape("line")}     title="Line"><Slash className={ico}/></button>
                </div>
            ...
            {selectedKind === "text" && (
              <div className="space-y-2">
                ...
                {/* textarea: до 3 строк, обычный инпут без экзотики */}
                <textarea
                  rows={2}
                  maxLength={500}
                  value={selectedProps?.text ?? ""}
                  onChange={(e)=> setSelectedText(e.target.value)}
                  placeholder="Введите текст… (Shift+Enter — перенос)"
                  className="w-full border px-2 py-2 text-sm rounded-none resize-none"
                ...
                <div className="text-[11px] text-black/60 leading-snug">
                  Совет: угловыми ручками масштабируйте шрифт — размер пересчитывается без «пиксельного скейла».
                  Боковые ручки регулируют ширину блока (перенос строк).
                </div>
              </div>
            )}

            {/* большие переключатели FRONT/BACK и отдельные Download */}
            <div className="space-y-2">
              <div className="flex gap-2">
                <button
                  className={clx(
                    "flex-1 h-10 border rounded-none uppercase tracking-wide font-bold",
                ...
                <button
                  className="w-[120px] h-10 border rounded-none uppercase tracking-wide font-bold flex items-center justify-center gap-2"
                  style={{ fontFamily: SYS }}
  ...
  const onFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (f) onUploadImage(f)
    e.currentTarget.value = ""
    setOpen(false) // автозакрытие шторки после вставки
  }

  return (
    <>
      {/* Нижняя кнопка */}
      <div className="fixed left-0 right-0 bottom-0 z-40 grid place-items-center pointer-events-none" style={{ fontFamily: SYS }}>
        <div className="pointer-events-auto mb-[env(safe-area-inset-bottom,12px)]">
          <button
            className="px-6 h-12 min-w-[160px] bg-black text-white text-sm tracking-wide uppercase rounded-none shadow-lg active:scale-[.98] transition font-bold"
      ...
      {/* Шторка */}
      {open && (
        <div className="fixed inset-0 z-50" onClick={()=>setOpen(false)} style={{ fontFamily: SYS }}>
          ...
          >
            <div className="flex items-center justify-between px-3 pt-2 pb-1">
              <div className="flex gap-1">
                <button
                  className={clx("px-3 h-9 border text-xs rounded-none font-bold", tab==="tools" ? "bg-black text-white" : "bg-white")}
                  onClick={()=>setTab("tools")}
                >
                  Tools
                </button>
                <button
                  className={clx("px-3 h-9 border text-xs rounded-none font-bold", tab==="layers" ? "bg-black text-white" : "bg-white")}
              ...
              {tab === "tools" && (
                <>
                  <div className="grid grid-cols-6 gap-0 rounded-none overflow-hidden">
                    ...
                    <button className={clx(iconBtn, tool==="brush" && "bg-black text-white")} onClick={()=>setTool("brush")} title="Brush"><Brush className={ico}/></button>
                    <button className={clx(iconBtn, tool==="erase" && "bg-black text-white")} onClick={()=>setTool("erase")} title="Eraser"><Eraser className={ico}/></button>
                    <button className={iconBtn} onClick={()=>{ onAddText(); setOpen(false) }} title="Text"><TypeIcon className={ico}/></button>
                    <button className={clx(iconBtn, tool==="shape" && "bg-black text-white")} onClick={()=>setTool("shape")} title="Shapes"><Shapes className={ico}/></button>
                    <button className={iconBtn} onClick={()=>fileRef.current?.click()} title="Image"><ImageIcon className={ico}/></button>
                  </div>
                  ...
                  {tool==="shape" && (
                    <div className="grid grid-cols-5 gap-0 rounded-none overflow-hidden">
                      <button className={iconBtn} onClick={()=>{ onAddShape("circle"); setOpen(false) }}   title="Circle"><Circle className={ico}/></button>
                      <button className={iconBtn} onClick={()=>{ onAddShape("square"); setOpen(false) }}   title="Square"><Square className={ico}/></button>
                      <button className={iconBtn} onClick={()=>{ onAddShape("triangle"); setOpen(false) }} title="Triangle"><Triangle className={ico}/></button>
                      <button className={iconBtn} onClick={()=>{ onAddShape("cross"); setOpen(false) }}    title="Cross"><Plus className={ico}/></button>
                      <button className={iconBtn} onClick={()=>{ onAddShape("line"); setOpen(false) }}     title="Line"><Slash className={ico}/></button>
                    </div>
                  )}
