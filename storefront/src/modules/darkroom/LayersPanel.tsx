...
import { Eye, EyeOff, Lock, Unlock, Copy, Trash2, X } from "lucide-react"
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between px-4 py-3">
          <div className="text-[12px] uppercase tracking-wide">Layers</div>
          <button onClick={onClose} className="w-9 h-9 grid place-items-center border border-black/60 rounded-none"><X className="w-4 h-4"/></button>
        </div>
        <div className="max-h-[45vh] overflow-auto p-2 space-y-1">
          {items.map((it) => (
            <div
              key={it.id}
            ...
            >
              ...
              <select
                className={clx(
                  "h-8 px-1 border rounded-none text-xs w-24",
              ...
              >
                {blendOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
              </select>
  ...
  return (
    <div className="fixed right-6 top-40 z-40 w-[360px] border border-black/10 bg-white/95 shadow-xl rounded-none">
      ...
      <div className="max-h-[62vh] overflow-auto p-2 space-y-1">
        {/* …ровно тот же список, как в mobile-ветке выше … */}
        {/* (оставил полностью тот же JSX, чтобы поведение совпадало) */}
        {items.map((it) => (
          <div
            key={it.id}
            ref={(el) => (rowRefs.current[it.id] = el)}
          ...
          >
            ...
            <div className="text-xs flex-1 truncate">{it.name}</div>
            <select
              className={clx("h-8 px-1 border rounded-none text-xs w-24", selectId === it.id ? "bg-black text-white border-white/40" : "bg-white")}
              value={it.blend}
              onChange={(e) => onChangeBlend(it.id, e.target.value)}
              onMouseDown={(e)=>e.stopPropagation()}
            >
