      ...
      <div className="max-h-[62vh] overflow-auto p-2 space-y-1">
        {items.map((it) => (
          <div
            ...
            title={it.name}
          >
            {/* drag-handle только здесь */}
            <button
              className="w-8 h-8 grid place-items-center border border-current bg-transparent cursor-grab active:cursor-grabbing"
              draggable
            ...
            <div className="w-24" onMouseDown={(e)=>e.stopPropagation()}>
              <input
                type="range" min={5} max={100}
                value={Math.round(it.opacity * 100)}
                onChange={(e)=>onChangeOpacity(it.id, Math.max(0.05, parseInt(e.target.value)/100))}
