...
export default function EditorCanvas() {
  ...
  useLayoutEffect(() => {
    const el = (document.querySelector("header") || document.getElementById("site-header")) as
      | HTMLElement
      | null
  ...
  const textSnap = useRef<{ width: number; height: number; fs: number; cx: number; cy: number } | null>(
    null,
  )
  ...
  const attachTransformer = () => {
    ...
    if (isTextNode(n)) {
      ...
      const onTextStart = () => {
        textSnap.current = makeTextSnap(n)
      }
      const onTextEnd = () => {
      ...
      ;(tr as any).boundBoxFunc((oldBox: any, newBox: any) => {
        ...
        const active =
          trRef.current && (trRef.current as any).getActiveAnchor
            ? (trRef.current as any).getActiveAnchor()
            : undefined

        // БОКОВЫЕ — только ширина (wrap), центр сохраняем
        ...
        // Углы/вертикаль — меняем fontSize, сохраняем центр
        const ratioW = newBox.width / Math.max(1e-6, snap.width)
        ...
        if (Math.abs(t.fontSize() - nextFS) > EPS) {
          ...
          const newH = Math.max(
            1,
            (self2 && typeof self2.height === "number") ? self2.height : (t.height() || snap.height),
          )
  ...
  const onAddShape = (kind: ShapeKind) => {
    ...
    if ((n as any).zIndex) (n as any).zIndex(nextTopZ())
  ...
  // ===== Координаты указателя (ОБЩИЕ, без дублей) =====
  ...
  // ===== Overlay-редактор текста (двойной клик) =====
  ...
  const duplicateLayer = (id: string) => {
    ...
    const newLay: AnyLayer = {
      id: (clone as any).id ? (clone as any).id() : uid(),
      node: clone,
      side: src.side,
  ...
  const reorder = (srcId: string, destId: string, place: "before" | "after") => {
    setLayers((prev) => {
      ...
      bottomToTop.forEach((l, i) => {
        ;(l.node as any).zIndex && (l.node as any).zIndex(i)
      })
      artLayerRef.current?.batchDraw()
  ...
  const setSelectedStroke = (hex: string) => {
    const n = sel?.node as any
    if (!n || typeof n.stroke !== "function") return
    n.stroke(hex)
    artLayerRef.current?.batchDraw()
    bump()
  }
  const setSelectedStrokeW = (w: number) => {
    const n = sel?.node as any
    if (!n || typeof n.strokeWidth !== "function") return
    n.strokeWidth(w)
    artLayerRef.current?.batchDraw()
    bump()
  }
  ...
  const setSelectedColor = (hex: string) => {
    ...
    if (sel.type === "text") {
      ...
      if (n instanceof Konva.Group) {
        n.find((child: any) => child instanceof Konva.Rect || child instanceof Konva.Circle || child instanceof Konva.RegularPolygon || child instanceof Konva.Line).forEach(
          (child: any) => {
