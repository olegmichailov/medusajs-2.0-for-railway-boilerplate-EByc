...
export default function EditorCanvas() {
  const { side, set, tool, brushColor, brushSize, shapeKind, selectedId, select,
          showPanel, togglePanel, showLayers, toggleLayers } = useDarkroom()
  ...
  const { viewW, viewH, scale } = useMemo(() => {
    ...
    const sidePanel = !isMobile ? 320 : 0
  ...
  const attachTransformer = () => {
    ...
    if (node && tRef.current && !(node as any).attrs?.locked) {
  ...
  useEffect(()=>{
    const onKey = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey||e.ctrlKey) && e.key.toLowerCase()==="d") {
        ...
        const lay = layers.find(l=>l.id===selectedId)!
        const meta = { ...lay.meta, name: lay.meta.name+" copy" }
        setLayers(p=>[...p, { id: (clone as any)._id, node: clone, side, meta, type: lay.type }])
  ...
  const cycleBlend = (dir:1|-1) => {
    ...
    setLayers(p=>p.map(l=> l.id===selectedId ? (applyMeta(l.node, {...l.meta, blend: next}), {...l, meta:{...l.meta, blend: next}}) : l))
  }
  ...
  const addShape = (kind: ShapeKind) => {
    ...
    applyMeta(node, meta)
    const id = (node as any)._id
    setLayers(p=>[...p, { id, side, node, meta, type: kind==="line" ? "shape" : "shape"}])
    select(id); set({ tool: "move" })
  }
  ...
  const appendStroke = (x:number,y:number) => {
    ...
    node.points(pts); node.getLayer()?.batchDraw()
  }

  // ——— Crop (рамка = отдельный rect + transformer; Apply — к выбранному ноду)
  const startCrop = () => {
    const node = findNode(selectedId)
    if (!node) return
    setIsCropping(true)
  ...
  const onDelete = (id:string)=> { setLayers(p=>p.filter(l=>l.id!==id)); if (selectedId===id) select(null) }
  ...
  const onMoveUp = (id:string)=> { const node = layers.find(l=>l.id===id)?.node; node?.moveUp(); node?.getLayer()?.batchDraw() }
  const onMoveDown = (id:string)=> { const node = layers.find(l=>l.id===id)?.node; node?.moveDown(); node?.getLayer()?.batchDraw() }
    ...
    <div className="relative w-screen h-[calc(100vh-80px)] overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        ...
        >
          ...
          <Layer>
            {/* живые ноды уже в памяти — заставляем React «трогать» их, чтобы конва перерисовывала */}
            {visLayers.map((l)=>(
              <Group
                key={l.id}
                onClick={()=> select(l.id)}
