...
type FxParams = {
  ...
  angle: number     // deg
  dot: number       // 0..1 (заполнение)
  palette: string[] // 5 HEX
}
const DEFAULT_FX: FxParams = {
  enabled: false, live: true, cell: 10, levels: 4, angle: 45, dot: 0.7,
  palette: ["#EAEAEA","#FFD447","#00C2FF","#FF4DA6","#2B2B2B"],
}
// WebGL2 шейдер
...
uniform float u_cell, u_levels, u_angle, u_dot;
uniform vec3  u_pal[5];
float luma(vec3 c){ return dot(c, vec3(0.2126,0.7152,0.0722)); }
vec3 posterizePal(float g){
  float stepv = 1.0/max(u_levels,1.0);
  float t0=stepv*1.0, t1=stepv*2.0, t2=stepv*3.0, t3=stepv*4.0;
  if (g < t0) return u_pal[4];
...
void main(){
  ...
  // чем светлее — тем меньше «чернил»
  float dots = smoothstep(u_dot, 0.0, r);
  vec3 baseCol = posterizePal(g);
  vec3 col = mix(baseCol, baseCol*0.15, dots); // мягкая тень от растра
...
export default function EditorCanvas() {
  ...
  useEffect(() => { if (isMobile) set({ tool: "brush" as Tool }); (Konva as any).hitOnDragEnabled = true }, [set])
  ...
  const toolsAnchorRef = useRef<HTMLDivElement>(null) // якорь под Toolbar
  ...
  const ensureGL = useCallback(() => {
    ...
    glProgRef.current = prog; gl.useProgram(prog)
    ...
    glUniformsRef.current = {
      u_image: uni("u_image"), u_resolution: uni("u_resolution"), u_cell: uni("u_cell"),
      u_levels: uni("u_levels"), u_angle: uni("u_angle"), u_dot: uni("u_dot"),
      u_pal0: uni("u_pal[0]"), u_pal1: uni("u_pal[1]"), u_pal2: uni("u_pal[2]"),
      u_pal3: uni("u_pal[3]"), u_pal4: uni("u_pal[4]"),
    }
    return true
  }, [])
  ...
  const renderFx = useCallback(() => {
    ...
    const canvas = fxCanvasRef.current
    const konvaCanvas =
      (artLayerRef.current as any)?.getNativeCanvasElement?.() ||
      (artLayerRef.current as any)?.getCanvas?.()?._canvas
    if (!gl || !canvas || !konvaCanvas) return

    // размеры: полноразмерная сцена, CSS — под текущий scale
    canvas.width = BASE_W; canvas.height = BASE_H
    // viewport
    gl.viewport(0,0,canvas.width,canvas.height)
    gl.useProgram(glProgRef.current!)

    // текстура из внутреннего canvas Konva (без CPU копий)
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, konvaCanvas)

    const U = glUniformsRef.current
    gl.uniform1i(U.u_image!, 0)
    gl.uniform2f(U.u_resolution!, BASE_W, BASE_H)
    gl.uniform1f(U.u_cell!, fx.cell)
    gl.uniform1f(U.u_levels!, Math.max(2, Math.min(6, fx.levels)))
  ...
  const applyMeta = (n: AnyNode, meta: BaseMeta) => { n.opacity(meta.opacity); (n as any).globalCompositeOperation = meta.blend }
    ...
    if (isTextNode(n)) {
      ...
      const onStartText = () => {
        ...
        textStart.current = {
          ...
          left: t.x(), right: t.x() + (t.width() || 0),
          fontSize: t.fontSize(),
          anchor: a === "middle-left" || a === "middle-right" ? (a as any) : "corner"
        }
      ...
      const onTransform = () => {
        ...
        t.getLayer()?.batchDraw(); trRef.current?.forceUpdate(); uiLayerRef.current?.batchDraw()
      }

      const onEnd = () => { t.scaleX(1); t.scaleY(1); textStart.current = null; t.getLayer()?.batchDraw() }
      ...
      const onTransform = () => {
        ...
        const isCorner = active && ["top-left","top-right","bottom-left","bottom-right"].includes(active)
      ...
      n.on("transform.fix", onTransform); n.on("transformend.fix", onEnd)
  ...
  // перетаскивание: во время brush/erase отключаем
  useEffect(() => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      n.getLayer()?.batchDraw(); if (fx.enabled && fx.live) requestFx()
    }
  ...
  const createStrokeGroup = (): AnyLayer => {
    const g = new Konva.Group({ x: 0, y: 0 }); (g as any)._isStrokes = true; g.id(uid())
    const id = g.id(); const meta = baseMeta(`strokes ${seqs.strokes}`)
    currentArt().add(g); g.zIndex(nextTopZ())
    const newLay: AnyLayer = { id, side, node: g, meta, type: "strokes" }
    setLayers(p => [...p, newLay]); setSeqs(s => ({ ...s, strokes: s.strokes + 1 }))
    currentStrokeId.current[side] = id; return newLay
  }
  const createEraseGroup = (): AnyLayer => {
    const g = new Konva.Group({ x: 0, y: 0 }); (g as any)._isErase = true; g.id(uid())
    const id = g.id(); const meta = baseMeta(`erase ${seqs.erase}`)
    currentArt().add(g); g.zIndex(nextTopZ())
    const newLay: AnyLayer = { id, side, node: g as AnyNode, meta, type: "erase" }
    setLayers(p => [...p, newLay]); setSeqs(s => ({ ...s, erase: s.erase + 1 }))
    currentEraseId.current[side] = id; return newLay
  }
  const siteFont = () =>
  ...
  const onUploadImage = (file: File) => {
    ...
    r.onload = () => {
      ...
      img.onload = () => {
        ...
        ;(kimg as any).setAttr("src", r.result as string); kimg.id(uid()); const id = kimg.id()
        const meta = baseMeta(`image ${seqs.image}`)
        currentArt().add(kimg); attachCommonHandlers(kimg, id)
        setLayers(p => [...p, { id, side, node: kimg, meta, type: "image" }])
        setSeqs(s => ({ ...s, image: s.image + 1 })); select(id)
        artLayerRef.current?.batchDraw(); set({ tool: "move" })
  ...
  const onAddText = () => {
    ...
    t.id(uid()); const id = t.id(); const meta = baseMeta(`text ${seqs.text}`)
    currentArt().add(t); attachCommonHandlers(t, id)
    setLayers(p => [...p, { id, side, node: t, meta, type: "text" }])
    setSeqs(s => ({ ...s, text: s.text + 1 })); select(id)
    artLayerRef.current?.batchDraw(); set({ tool: "move" })
    if (fx.enabled && fx.live) requestFx()
  }
  ...
  const onAddShape = (kind: ShapeKind) => {
    ...
    ;(n as any).id(uid()); const id = (n as any).id?.() ?? uid()
    const meta = baseMeta(`shape ${seqs.shape}`); currentArt().add(n as any)
    attachCommonHandlers(n, id)
    setLayers(p => [...p, { id, side, node: n, meta, type: "shape" }])
    setSeqs(s => ({ ...s, shape: s.shape + 1 })); select(id)
    artLayerRef.current?.batchDraw(); set({ tool: "move" })
    if (fx.enabled && fx.live) requestFx()
  }
  ...
  const startStroke = (x: number, y: number) => {
    if (tool === "brush") {
      let gid = currentStrokeId.current[side]; if (!gid) gid = createStrokeGroup().id
      ...
      let gid = currentEraseId.current[side]; if (!gid) gid = createEraseGroup().id
      const g = find(gid)!.node as Konva.Group
  ...
  const appendStroke = (x: number, y: number) => {
    if (!isDrawing) return
    if (tool === "brush") {
      const gid = currentStrokeId.current[side]; const g = gid ? (find(gid)?.node as Konva.Group) : null
      const last = g?.getChildren().at(-1); const line = last instanceof Konva.Line ? last : (last as Konva.Group)?.getChildren().at(-1)
      if (!(line instanceof Konva.Line)) return
      line.points(line.points().concat([x, y])); artLayerRef.current?.batchDraw()
    } else if (tool === "erase") {
      const gid = currentEraseId.current[side]; const g = gid ? (find(gid)?.node as Konva.Group) : null
      const last = g?.getChildren().at(-1) as Konva.Line | undefined
      if (!(last instanceof Konva.Line)) return
      last.points(last.points().concat([x, y])); artLayerRef.current?.batchDraw()
    }
    if (fx.enabled && fx.live) requestFx()
  }
  ...
  const startTextOverlayEdit = (t: Konva.Text) => {
    const stage = stageRef.current!; const stContainer = stage.container()
    if (editingRef.current) { editingRef.current.cleanup(); editingRef.current = null }

    const prevOpacity = t.opacity(); t.opacity(0.001); t.getLayer()?.batchDraw()
    const ta = document.createElement("textarea"); ta.value = t.text()
    Object.assign(ta.style, {
      position: "absolute", padding: "0", margin: "0", border: "none", background: "transparent",
      color: String(t.fill() || "#000"), fontFamily: t.fontFamily(),
      ...
      transformOrigin: "left top", zIndex: "9999", userSelect: "text", caretColor: String(t.fill() || "#000"),
    ...
    document.body.appendChild(ta); sync(); ta.focus(); ta.setSelectionRange(ta.value.length, ta.value.length)

    const onInput = () => { t.text(ta.value); t.getLayer()?.batchDraw(); trRef.current?.forceUpdate(); uiLayerRef.current?.batchDraw(); requestAnimationFrame(sync); if (fx.enabled && fx.live) requestFx() }
    const cleanup = (apply = true) => {
      window.removeEventListener("scroll", onScroll, true); window.removeEventListener("resize", onResize)
      t.off(".edit"); ta.removeEventListener("input", onInput); ta.removeEventListener("keydown", onKey)
      if (apply) t.text(ta.value); ta.remove(); t.opacity(prevOpacity); t.getLayer()?.batchDraw()
    ...
    const onResize = () => sync(); const onScroll = () => sync()
  ...
  const isTransformerChild = (t: Konva.Node | null) => {
    ...
    while (p) { if (tr && p === tr) return true; p = p.getParent?.() } return false
  }
  ...
  const onDown = (e: any) => {
    e.evt?.preventDefault?.(); if (isTransformerChild(e.target)) return
    if (tool === "brush" || tool === "erase") { const sp = getStagePointer(); const p = toCanvas(sp); startStroke(p.x, p.y); return }
    ...
    if (tgt && tgt !== st && tgt.getParent()) { const found = layers.find(l => l.node === tgt || l.node === (tgt.getParent() as any)); if (found && found.side === side) select(found.id) }
  }
  const onMove = (e: any) => {
    if (isTransformingRef.current) return; if (isTransformerChild(e.target)) return
    if (!isDrawing) return; const p = toCanvas(getStagePointer()); appendStroke(p.x, p.y)
  }
  ...
  const layerItems: LayerItem[] = useMemo(() => {
    return layers
      ...
      .reverse()
      .map(l => ({ id: l.id, name: l.meta.name || l.type, type: l.type, visible: l.meta.visible, locked: l.meta.locked, blend: l.meta.blend, opacity: l.meta.opacity }))
  }, [layers, side])
  ...
  const deleteLayer = (id: string) => {
    ...
    if (selectedId === id) select(null); artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  const duplicateLayer = (id: string) => {
    const src = layers.find(l => l.id===id); if (!src) return
    const clone = src.node.clone() as AnyNode
    ;(clone as any).x((src.node as any).x?.() + 20); (clone as any).y((src.node as any).y?.() + 20)
    ;(clone as any).id(uid()); currentArt().add(clone as any)
    ...
    attachCommonHandlers(clone, newLay.id); setLayers(p => [...p, newLay]); select(newLay.id)
    clone.zIndex(nextTopZ()); artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  const reorder = (srcId: string, destId: string, place: "before" | "after") => {
    ...
    select(srcId); requestAnimationFrame(attachTransformer); if (fx.enabled && fx.live) requestFx()
  }
  const updateMeta = (id: string, patch: Partial<BaseMeta>) => {
    ...
    artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  ...
  const selectedProps =
    sel && isTextNode(sel.node) ? { text: sel.node.text(), fontSize: sel.node.fontSize(), fontFamily: sel.node.fontFamily(), fill: sel.node.fill() as string } :
    sel && (sel.node as any).fill ? { fill: (sel.node as any).fill() ?? "#000000", stroke: (sel.node as any).stroke?.() ?? "#000000", strokeWidth: (sel.node as any).strokeWidth?.() ?? 0 } : {}
  ...
  const setSelectedColor      = (hex:string)  => {
    if (!sel) return; const n = sel.node as any
  ...
  const clearArt = () => {
    ...
    g.removeChildren(); setLayers(prev => prev.filter(l => l.side !== side))
    currentStrokeId.current[side] = null; currentEraseId.current[side]  = null
    select(null); artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  ...
  const downloadBoth = async (s: Side) => {
    ...
    uiLayerRef.current?.visible(true); artLayerRef.current?.opacity(prevArtOpacity); st.draw()
  ...
  // FX lifecycle — гасим арт только если GL готов
  useEffect(() => {
    if (!fxCanvasRef.current) return
    if (fx.enabled) {
      const ok = ensureGL()
      if (!ok) { setFx(f => ({ ...f, enabled: false })); return }
      artLayerRef.current?.opacity(0)
      fxCanvasRef.current.style.opacity = "1"
      requestFx()
    } else {
      artLayerRef.current?.opacity(1)
      const gl = glRef.current; if (gl) { gl.clearColor(0,0,0,0); gl.clear(gl.COLOR_BUFFER_BIT) }
      if (fxCanvasRef.current) fxCanvasRef.current.style.opacity = "0"
    }
    uiLayerRef.current?.batchDraw()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fx.enabled])

  useEffect(() => { if (fx.enabled && fx.live) requestFx() }, [fx.cell, fx.levels, fx.angle, fx.dot, fx.palette, fx.live, requestFx])

  /* ====== Геометрия тулбара для привязки FX-панели ====== */
  const [toolsBox, setToolsBox] = useState<{left:number; top:number; bottom:number; width:number} | null>(null)
  useEffect(() => {
    const read = () => {
      const el = toolsAnchorRef.current
      if (!el) return
      const r = el.getBoundingClientRect()
      setToolsBox({ left: r.left, top: r.top, bottom: r.bottom, width: r.width })
    }
    read()
    const onResize = () => read()
  ...
  return (
    ...
    >
      {/* стили фейдеров: плоские, квадратные */}
      <style
        dangerouslySetInnerHTML={{
          __html: `
            [data-fxpanel] input[type="range"]{
              -webkit-appearance:none; width:100%; height:4px; background:#e5e5e5; border:1px solid #111; border-width:1px 0; outline:none; margin:6px 0;
            }
            [data-fxpanel] input[type="range"]::-webkit-slider-thumb{
              -webkit-appearance:none; width:12px; height:12px; background:#111; border:1px solid #111; border-radius:0; margin-top:-4px;
            }
            [data-fxpanel] input[type="checkbox"]{
              appearance:none; width:12px; height:12px; border:1px solid #111; margin:0; display:inline-block; vertical-align:middle;
            }
            [data-fxpanel] input[type="checkbox"]:checked{ background:#111; }
            [data-fxpanel] .btn { height:24px; padding:0 8px; font-size:12px; border:1px solid #111; background:#fff; }
            [data-fxpanel] .btn.primary { background:#111; color:#fff; }
          `
        }}
      />
      ...
      <div className="w-full h-full flex items-start justify-center">
        <div style={{ position: "relative", touchAction: "none", width: viewW, height: viewH }}>
          ...
          {/* FX overlay canvas (GPU), не перехватывает клики */}
          <canvas
            ref={fxCanvasRef}
            style={{
      ...
      {/* TOOLBAR с якорем для FX-панели */}
      ...
      {/* === EFFECTS PANEL (строго под TOOLBAR, плоский стиль) === */}
      <div
        data-fxpanel
        style={toolsBox ? {
          position: "fixed",
          left: toolsBox.left,
          top:  toolsBox.bottom + 8,
      ...
      >
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
          <div style={{ fontSize:12, fontWeight:700, letterSpacing:.2 }}>EFFECTS</div>
          <div style={{ display:"flex", gap:6 }}>
            <button
              onClick={()=>setFxOpen(v=>!v)}
              className="btn"
              style={{ width:24, textAlign:"center" }}
              title={fxOpen ? "Collapse" : "Expand"}
            >{fxOpen ? "–" : "+"}</button>
            <button
              onClick={()=>setFx(p=>({...p, enabled: !p.enabled}))}
              className={`btn ${fx.enabled ? "primary":""}`}
              title="On/Off"
            >FX</button>
          </div>
        </div>
        ...
        {fxOpen && (
          <>
            ...
            <Range label="Cell"   min={4}  max={64} step={1}   value={fx.cell}   onChange={v=>setFx(p=>({...p, cell:v}))} />
            <Range label="Levels" min={2}  max={6}  step={1}   value={fx.levels} onChange={v=>setFx(p=>({...p, levels:v}))} />
            <Range label="Angle"  min={0}  max={90} step={1}   value={fx.angle}  onChange={v=>setFx(p=>({...p, angle:v}))} format={v=>`${v}°`} />
            <Range label="Dot"    min={0}  max={1}  step={0.01} value={fx.dot}    onChange={v=>setFx(p=>({...p, dot:v}))}   format={v=>`${Math.round(v*100)}%`} />

            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:6}}>
              <div style={{fontSize:12, opacity:.8}}>Palette</div>
              <div style={{display:"flex", gap:6}}>
                {fx.palette.map((c, i)=>(
                  <button key={i}
                    onClick={()=> {
            ...
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:8}}>
              ...
              <div style={{display:"flex", gap:6}}>
                <button className="btn primary" onClick={()=>{ if (!fx.enabled) return; ensureGL(); renderFx() }}>Update</button>
                <button className="btn" onClick={()=>setFx(DEFAULT_FX)}>Reset</button>
              </div>
            </div>
          </>
        )}
...
function Range({label,min,max,step,value,onChange,format}:{label:string;min:number;max:number;step:number;value:number;onChange:(v:number)=>void;format?:(v:number)=>string}) {
  return (
