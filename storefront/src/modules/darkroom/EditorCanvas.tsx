...
vec3 posterizePal(float g){
  // g in 0..1, thresholds equally spaced by u_levels
...
void main(){
  ...
  // halftone mask
  vec2 px = uv * u_resolution;          // pixels
  vec2 pr = rot(px, u_angle) / max(u_cell,1.0);
  vec2 f = fract(pr)-0.5;
  float r = length(f*2.0);               // distance to cell center
  float dotMask = smoothstep(u_dot, 0.0, r); // more g -> smaller dot
  float ink = mix(1.0, dotMask, 0.8);    // ink darkness

  // posterize + palette
  vec3 baseCol = posterizePal(g);

  // multiply by ink (screenprint dark)
...
export default function EditorCanvas() {
  ...
  useEffect(() => {
    ...
    attachTransformer()
    // FX: если включен — обновить из текущей стороны
    if (fx.enabled) requestFx()
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [side, layers])

  // ===== Transformer: anti-jitter текст =====
  ...
  const attachTransformer = () => {
    ...
    if (isTextNode(n)) {
      ...
      // ключ: отменяем стандартное применение трансформа на узел (через boundBox),
      // а сами внутри onTransform применяем либо width, либо fontSize, затем сбрасываем scale=1
      tr.boundBoxFunc((oldB, newB) => {
        const a = (trRef.current as any)?.getActiveAnchor?.() as string | undefined
        if (!a) return newB
        if (a.startsWith("middle") || a.includes("left") || a.includes("right")) return oldB
        return oldB
      })
  ...
  // ==== FX lifecycle: hide art (opacity 0) + draw overlay ====
  useEffect(() => {
    if (!fxCanvasRef.current) return
    if (fx.enabled) {
      if (!ensureGL()) return
      // делаем арт прозрачным (для визуала) — но он остаётся интерактивным
  ...
  return (
    ...
    >
      ...
      {/* === EFFECTS PANEL (карточка в стиле тулбара) === */}
      <div
        style={{
          position: "fixed",
          left: 24, bottom: 24 + padBottom - (isMobile ? 72 : 0),
          background: "#fff", border: "1px solid #e5e7eb",
          borderRadius: 12, boxShadow: "0 6px 20px rgba(0,0,0,.06)",
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: 8}}>
          ...
          <label style={{display:"flex", alignItems:"center", gap:8}}>
            ...
            <input
              type="checkbox"
              checked={fx.enabled}
              onChange={e=>setFx(v=>({...v, enabled: e.target.checked}))}
            />
          </label>
        </div>

        {/* Controls (line faders) */}
        <Control label="Mode" right="ScreenPrint+" />

        <Range label="Cell"  min={4} max={64} step={1} value={fx.cell}
               onChange={v=>setFx(p=>({...p, cell:v}))} />
        <Range label="Levels" min={2} max={6} step={1} value={fx.levels}
               onChange={v=>setFx(p=>({...p, levels:v}))} />
        <Range label="Angle" min={0} max={90} step={1} value={fx.angle}
               onChange={v=>setFx(p=>({...p, angle:v}))} format={v=>`${v}°`} />
        <Range label="Dot"   min={0} max={1} step={0.01} value={fx.dot}
               onChange={v=>setFx(p=>({...p, dot:v}))} format={v=>`${Math.round(v*100)}%`} />
          ...
          <div style={{display:"flex", gap:8}}>
            {fx.palette.map((c, i)=>(
              <button key={i}
                ...
                title={c}
                style={{width:20, height:20, borderRadius:6, border:"1px solid #d1d5db", background:c}}
              />
            ))}
          </div>
        </div>
...
/* ===== Small UI helpers for panel (без внешних зависимостей) ===== */
function Control({label,right}:{label:string;right?:string}) {
  return (
