...
/* ---------------- UI HELPERS (ПОДНЯТЫ ВВЕРХ) ---------------- */
function FxRow({label,right}:{label:string;right?:string}) {
  return (
...
const btnStyle: React.CSSProperties = {
  ...
  background:"#111", color:"#fff", borderRadius:8, border:"1px solid #111"
}
...
export default function EditorCanvas() {
  ...
  const ensureGL = useCallback(() => {
    ...
    try {
      ...
      uniformsRef.current = {
        u_image: u("u_image"), u_resolution: u("u_resolution"),
        u_cell: u("u_cell"), u_levels: u("u_levels"),
        u_angle: u("u_angle"), u_dot: u("u_dot"),
        u_pal0: u("u_pal[0]"), u_pal1: u("u_pal[1]"), u_pal2: u("u_pal[2]"), u_pal3: u("u_pal[3]"), u_pal4: u("u_pal[4]")
      }
      return true
    } catch (e) {
  ...
  const renderFx = useCallback(() => {
    ...
    const gl = glRef.current, canvas = fxCanvasRef.current
    const sceneCanvas = (artLayerRef.current?.getCanvas() as any)?.getCanvas?.() as HTMLCanvasElement | undefined
      || (artLayerRef.current?.getCanvas() as any)?._canvas as HTMLCanvasElement | undefined
    if (!gl || !canvas || !sceneCanvas) return
    ...
    const setPal = (i:number, hex:string) => {
      const [r,g,b]=hexToRGB(hex)
      gl.uniform3f((U as any)[`u_pal${i}`], r,g,b)
    }
    ...
    fx.palette.forEach((c,i)=>setPal(i,c))
  ...
  useEffect(() => {
    ...
    attachTransformer()
    if (fx.enabled) { ensureGL(); requestFx() }
  ...
  const attachTransformer = () => {
    ...
    if (isTextNode(n)) {
      ...
      tr.boundBoxFunc((oldB)=>oldB) // блокируем собственную геометрию — управляем сами
      ...
      const onTransform = () => {
        ...
        if (active === "middle-left" || active === "middle-right" || snap.anchor?.startsWith("middle")) {
          ...
          const fs = Math.max(TEXT_MIN_FS, Math.min(snap.fontSize * s, TEXT_MAX_FS))
          if (Math.abs(fs - t.fontSize()) > EPS) t.fontSize(fs)
          t.scaleX(1); t.scaleY(1)
        }
      ...
      const onEnd = () => {
        t.scaleX(1); t.scaleY(1); textStart.current = null
        t.getLayer()?.batchDraw()
      }
      ...
      const onTransform = () => {
        const active = (trRef.current as any)?.getActiveAnchor?.() as string | undefined
        let sx = (n as any).scaleX?.() ?? 1, sy = (n as any).scaleY?.() ?? 1
  ...
  const startTextOverlayEdit = (t: Konva.Text) => {
    const stage = stageRef.current!, stContainer = stage.container()
    ...
    const cleanup = (apply=true) => {
      ...
      select(find(selectedId)?.id ?? (t.id() as string)); requestAnimationFrame(()=>{ attachTransformer(); uiLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx() })
    }
    ...
    const onResize = () => sync(), onScroll = () => sync()
  ...
  const onMove = (e:any) => { if (isTransformingRef.current) return; if (isTransformerChild(e.target)) return; if (!isDrawing) return; const p = toCanvas(getStagePointer()); appendStroke(p.x,p.y) }
  const onUp = () => { if (isDrawing) finishStroke() }

  /* -------- Панель слоёв/мета -------- */
  const layerItems: LayerItem[] = useMemo(() =>
    layers.filter(l=>l.side===side).sort((a,b)=>a.node.zIndex() - b.node.zIndex()).reverse().map(l=>({
      id:l.id, name:l.meta.name||l.type, type:l.type, visible:l.meta.visible, locked:l.meta.locked, blend:l.meta.blend, opacity:l.meta.opacity
    })), [layers, side])

  const deleteLayer = (id: string) => {
    if (editingRef.current?.nodeId === id) { editingRef.current.cleanup(false); editingRef.current = null }
    setLayers(p => {
      const l = p.find(x => x.id===id); l?.node.destroy(); return p.filter(x => x.id!==id)
    })
  ...
  const duplicateLayer = (id:string) => {
    ...
    ;(clone as any).x((src.node as any).x?.()+20); (clone as any).y((src.node as any).y?.()+20); (clone as any).id(uid())
    ...
    attachCommonHandlers(clone, newLay.id); setLayers(p=>[...p,newLay]); select(newLay.id); clone.zIndex(nextTopZ()); artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  const reorder = (srcId:string, destId:string, place:"before"|"after") => {
    setLayers(prev => {
      const current=prev.filter(l=>l.side===side), others=prev.filter(l=>l.side!==side)
      const orderTopToBottom=current.slice().sort((a,b)=>a.node.zIndex()-b.node.zIndex()).reverse()
      const srcIdx=orderTopToBottom.findIndex(l=>l.id===srcId), dstIdx=orderTopToBottom.findIndex(l=>l.id===destId)
      if (srcIdx===-1||dstIdx===-1) return prev
      const src=orderTopToBottom.splice(srcIdx,1)[0]; const insertAt=place==="before"?dstIdx:dstIdx+1
      orderTopToBottom.splice(Math.min(insertAt, orderTopToBottom.length), 0, src)
      const bottomToTop=[...orderTopToBottom].reverse(); bottomToTop.forEach((l,i)=>{ (l.node as any).zIndex(i) }); artLayerRef.current?.batchDraw()
  ...
  const updateMeta = (id:string, patch:Partial<BaseMeta>) => {
    setLayers(p => p.map(l => {
      if (l.id!==id) return l
      const meta={...l.meta,...patch}; applyMeta(l.node, meta); if (patch.visible!==undefined) l.node.visible(meta.visible && l.side===side)
      return {...l, meta}
    }))
    artLayerRef.current?.batchDraw(); if (fx.enabled && fx.live) requestFx()
  }
  const onLayerSelect = (id:string) => { select(id); if (tool!=="move") set({ tool:"move" as Tool }) }
  ...
  const downloadBoth = async (s: Side) => {
    ...
    frontBgRef.current?.visible(showFront); backBgRef.current?.visible(!showFront ? true : false)
  ...
  useEffect(() => {
    ...
    if (fx.enabled) {
      if (!ensureGL()) return
      artLayerRef.current?.opacity(0) // арт остаётся интерактивным, просто скрыт визуально
      requestFx()
    } else {
      artLayerRef.current?.opacity(1)
      const gl = glRef.current; if (gl) gl.clear(gl.COLOR_BUFFER_BIT)
    }
  ...
  return (
    <div className="fixed inset-0 bg-white" style={{ paddingTop: padTop, paddingBottom: padBottom, overscrollBehavior:"none", WebkitUserSelect:"none", userSelect:"none" }}>
      ...
      <div className="w-full h-full flex items-start justify-center">
        <div style={{ position:"relative", touchAction:"none", width: viewW, height: viewH }}>
          ...
          >
            ...
            {/* 2. Арт (при FX — opacity:0, фильтр едет в WebGL оверлей) */}
      ...
      {/* Панель эффектов (стиль тулбара, без «FX»-кнопки) */}
      <div
        style={{
          position:"fixed", left:24, bottom:24 + padBottom - (isMobile?72:0),
          background:"#fff", border:"1px solid #e5e7eb", borderRadius:12, boxShadow:"0 6px 20px rgba(0,0,0,.06)",
          width:360, padding:12, zIndex:20
        }}
      >
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8}}>
          <div style={{fontWeight:600, letterSpacing:.2}}>Effects</div>
          <label style={{display:"flex", alignItems:"center", gap:8}}>
            <span style={{fontSize:12, opacity:.7}}>On</span>
            <input type="checkbox" checked={fx.enabled} onChange={e=>setFx(v=>({...v, enabled: e.target.checked}))}/>
          </label>
        </div>

        <FxRow label="Mode" right="ScreenPrint+" />
        <FxSlider label="Cell"   min={4}  max={64} step={1}    value={fx.cell}   onChange={v=>setFx(p=>({...p,cell:v}))} />
        <FxSlider label="Levels" min={2}  max={6}  step={1}    value={fx.levels} onChange={v=>setFx(p=>({...p,levels:v}))} />
        <FxSlider label="Angle"  min={0}  max={90} step={1}    value={fx.angle}  onChange={v=>setFx(p=>({...p,angle:v}))} format={v=>`${v}°`} />
        <FxSlider label="Dot"    min={0}  max={1}  step={0.01} value={fx.dot}    onChange={v=>setFx(p=>({...p,dot:v}))}    format={v=>`${Math.round(v*100)}%`} />
          ...
          <div style={{display:"flex", gap:8}}>
            {fx.palette.map((c,i)=>(
              <button key={i} onClick={()=>{
                ...
                title={c} style={{width:20,height:20,borderRadius:6,border:"1px solid #d1d5db",background:c}}
              />
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginTop:12}}>
          <label style={{display:"flex", alignItems:"center", gap:8}}>
            ...
            <span style={{fontSize:12, opacity:.8}}>Live</span>
          </label>
          <div style={{display:"flex", gap:8}}>
            <button onClick={()=>{ if (!fx.enabled) return; if (ensureGL()) renderFx() }} style={btnStyle}>Update</button>
            <button onClick={()=>setFx(DEFAULT_FX)} style={{...btnStyle, background:"#fff"}}>Reset</button>
          </div>
        </div>
      </div>
    </div>
